...
import { formDataDateTime } from "../../helpers/common";
const PostList = () => {
  ...
  const columns = [
    ...
    {
      name: "Created at",
      element: (row) =>formDataDateTime( row.created_at),
    },
    {
      name: "Updated at",
      element: (row) =>  formDataDateTime( row.updated_at),
    },
    {
      name: "Actions",
      element: (row) => (
  ...
  const requestDeleteApi = () => {
    if (deleteType == "single") {
      dispatch(actions.controlLoading(true));
      requestApi(`/post/${deleteItem}`, `DELETE`, [])
        .then((response) => {
          setShowModal(false);
          setRefresh(Date.now());
          dispatch(actions.controlLoading(false));
      ...
      dispatch(actions.controlLoading(true));
      requestApi(`/post/multiple?ids=${selectedRows.toString()}`, `DELETE`, [])
        .then((response) => {
          setShowModal(false);
          setRefresh(Date.now());
          setSelectedRows([]);
